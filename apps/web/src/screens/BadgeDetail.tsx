...
export function BadgeDetail({
  ...
  const distanceLabel = group?.distanceLabel ?? resolveDistanceLabel(badge.inputs.distance)
  ...
  const onDownload = () => {
    ...
    if (svg) void exportBadgePng(svg as SVGSVGElement, safeName(raceName) + '-badge.png')
  }
  ...
  return (
    <div
      style={{
        ...
        background: 'radial-gradient(120% 80% at 50% 0%, #262b20 0%, #1a1e16 60%, #14170f 100%)',
    ...
    >
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <button type="button" style={ghostBtn} onClick={() => onTweak(badge)}>
            ✎ Tweak
          </button>
            ...
            {menuOpen && (
              ...
              >
                <button
                  ...
                  onClick={() => {
                    setMenuOpen(false)
                    if (window.confirm(`Delete this badge for "${raceName}"? This can't be undone.`)) {
                      onDelete(badge)
                    }
                  }}
                >
      ...
      {generating && (
        <div style={{ maxWidth: 760, margin: '0 auto', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 18 }}>
          <div
          ...
          >
            <div
              className="tm-spin"
              style={{ width: 30, height: 30, border: '3px solid rgba(241,234,217,0.35)', borderTopColor: '#e9e3d4', borderRadius: '50%' }}
            />
          </div>
          <h1 style={{ fontFamily: FONT_DISP, fontWeight: 700, fontSize: 28, color: '#f1ead9', margin: 0, textAlign: 'center' }}>
            {raceName}
          </h1>
          <p style={{ color: '#aab09c', fontFamily: FONT_UI, fontSize: 14, margin: 0 }}>
      ...
      {failed && (
        <div style={{ maxWidth: 760, margin: '0 auto', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 18 }}>
          ...
          >
            ...
            <span style={{ color: '#e0a884', fontFamily: FONT_UI, fontWeight: 600, fontSize: 14, textAlign: 'center', padding: '0 16px' }}>
              {failLabel(badge)}
            </span>
          </div>
          <h1 style={{ fontFamily: FONT_DISP, fontWeight: 700, fontSize: 30, color: '#f1ead9', margin: 0, textAlign: 'center', lineHeight: 1.05 }}>
            {raceName}
          </h1>
          <p style={{ color: '#aab09c', fontFamily: FONT_UI, fontSize: 14, margin: 0, textAlign: 'center', maxWidth: 360 }}>
            This generation didn't finish. Retry to fix this badge in place, or tweak the design and try again.
          </p>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12, justifyContent: 'center' }}>
            ...
            <button type="button" style={{ ...ghostBtn, padding: '13px 24px', fontSize: 15 }} onClick={() => onTweak(badge)}>
              ✎ Tweak
            </button>
          </div>
        </div>
      ...
      >
        ...
        <div
          ref={svgWrapRef}
          style={{ position: 'relative', width: detailMedalW, maxWidth: '86vw' }}
        >
          {badge.keeper && (
            <span
              style={{
        ...
        <div style={{ textAlign: 'center' }}>
          ...
          >
            ...
            {finishTime && (
              <span>
                Finish{' '}
                <strong style={{ fontFamily: FONT_DISP, fontSize: 16, color: '#f1ead9' }}>
                  {finishTime}
                </strong>
              </span>
            )}
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12, justifyContent: 'center' }}>
          ...
          <button
            type="button"
            onClick={() => flashNote('Order flow — enamel pin · patch · medal (coming soon)')}
            style={{
        ...
        >
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, justifyContent: 'center' }}>
            <button type="button" style={shareBtn} onClick={() => onShare(badge)}>
              🔗 Copy link
            </button>
            <button type="button" style={shareBtn} onClick={() => flashNote('Opening Strava…')}>
              Strava
            </button>
            <button
              type="button"
              style={shareBtn}
              onClick={() => flashNote('Opening Instagram story…')}
            >
              Instagram
            </button>
          </div>
